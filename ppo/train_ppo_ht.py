...
def main():
	...
	# Comment or uncomment the environment on which you want to train the PPO model
	...
	for c in ParameterGrid(params):
		...
		# Training PPO model, based on a particular configuration of hyperparameters
		model = PPO("MlpPolicy", env, c.get("alpha"), verbose=0)
	...
	model = PPO("MlpPolicy", env, best_config.get("alpha"), verbose=0)
